(function (factory) {
  ...
  function mergeFromYMLFrontmatter(target, source) {
    ...
    if (source.submitted) {
      if (source.submitted instanceof Date) {
        target.submittedDate = source.submitted;
      } else if (source.submitted.constructor === String) {
        target.submittedDate = new Date(source.submitted);
      }
    }
    if (source.submittedDate) {
      if (source.submittedDate instanceof Date) {
        target.submittedDate = source.submittedDate;
      } else if (source.submittedDate.constructor === String) {
        target.submittedDate = new Date(source.submittedDate);
      } else {
        console.error('Don\'t know what to do with published date: ' + source.submittedDate);
      }
    }
  ...
  class FrontMatter {
    ...
    // 'Thu, 08 Sep 2016 00:00:00 -0700',
    get submittedDateRFC() {
      return RFC(this.submittedDate);
    }
    ...
    // 2016,
    get submittedYear() {
      return this.submittedDate.getFullYear();
    }

    // 'Sept',
    get submittedMonth() {
      return months[this.submittedDate.getMonth()];
    }

    // 8,
    get submittedDay() {
      return this.submittedDate.getDate();
    }

    // '09',
    get submittedMonthPadded() {
      return zeroPad(this.submittedDate.getMonth() + 1);
    }

    // '08',
    get submittedDayPadded() {
      return zeroPad(this.submittedDate.getDate());
    }

    get submittedISODateOnly() {
    ...
    assignToObject(target) {
      ...
      if (this.submittedDate) {
        target.volume = this.volume;
        target.issue = this.issue;
        target.submittedDateRFC = this.submittedDateRFC;
        target.submittedYear = this.submittedYear;
        target.submittedMonth = this.submittedMonth;
        target.submittedDay = this.submittedDay;
        target.submittedMonthPadded = this.submittedMonthPadded;
        target.submittedDayPadded = this.submittedDayPadded;
      }
      if (this.updatedDate) {
...
`, false);
  ...
  <div class="byline grid">
    ...
    <div>
      <h3>Submitted</h3>
      ${frontMatter.submittedDate ? `
        <p>${frontMatter.submittedMonth} ${frontMatter.submittedDay}, ${frontMatter.submittedYear}</p> ` : `
...
`;
  ...
  /* 0. Initialize directly (polyfill loading removed for security) */
  console.debug("Runlevel 0: No polyfills needed (modern browser assumed).");
